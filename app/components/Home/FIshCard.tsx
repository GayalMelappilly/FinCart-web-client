    ...
    return (
        ...
        >
            ...
            <div className={`relative rounded-xl overflow-hidden transition-all duration-300 h-full flex flex-col ${isFeatureFish
                ...
                <div className={`h-36 xs:h-40 sm:h-44 md:h-48 lg:h-52 overflow-hidden relative ${isFeatureFish ? 'bg-gradient-to-br from-amber-50 to-yellow-50' : ''
                    }`}>
                    {fish.images && fish.images.length > 0 ? (
                        <div className="w-full h-full relative">
                            ...
                            {!isGuest && (
                                <button
                                    className={`absolute top-3 right-3 backdrop-blur-sm transition-all duration-200 rounded-full p-2 z-10 shadow-sm transform ${isFeatureFish
                                ...
                                >
                                    <Heart size={18} className={`transition-all duration-300 ${mutation.isSuccess || isWishlisted ? 'text-red-500 fill-red-500' : ''}`} />
                                </button>
                            )}
                ...
                <div className={`p-3 xs:p-4 flex flex-col flex-grow ${isFeatureFish ? 'bg-gradient-to-br from-white via-amber-50/20 to-white' : ''
                    }`}>
                    <div className="mb-1.5">
                        <h3 className={`font-semibold text-sm xs:text-base sm:text-lg line-clamp-1 transition-colors ${isFeatureFish
                                ? 'text-gray-900 group-hover:text-amber-800'
                                : 'text-gray-800'
                            }`}>
                            {fish.name}
                        </h3>
                        <p className={`text-xs sm:text-sm line-clamp-1 ${isFeatureFish ? 'text-amber-700 font-medium' : 'text-gray-500'
                            }`}>
                            {fish.breed}
                        </p>
                    </div>
                    ...
                    <div className="mt-auto pt-1.5">
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-0.5">
                                {[...Array(5)].map((_, i) => (
                                    <Star
                                        key={i}
                                        className={`w-3 h-3 xs:w-3.5 xs:h-3.5 ${i < Math.round(fish.avgRating)
                        ...
                        <button
                            ...
                            flex items-center justify-center shadow-sm hover:shadow group cursor-pointer ${isFeatureFish
